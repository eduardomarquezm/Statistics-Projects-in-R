...
````{=tex}
\centerline{\fontsize{12}{17}\selectfont \textbf{Table 1: Descriptive Statistics of Selected Features}}
\vspace{-10pt}
\begin{singlespace}
...
summary_data <- df_summary %>%
  ...
  # 2. Pivot to make variables rows
...
````{=tex}
\begin{flushleft}
```{r response-vis, fig.width=8, out.width="100%", fig.asp=0.5, fig.cap="Normality Assessment on Area Mean", fig.pos="H"}

# The Histogram + Density Curve
p_hist <- ggplot(df, aes(x = Area_Mean)) +
  geom_histogram(aes(y = after_stat(density)), 
                 bins = 30, fill = "steelblue", alpha = 0.7, color = "white") +
  geom_density(color = "salmon", linewidth = 1) + # Using your 'Malignant' red for the line
  theme_minimal(base_size = 12, base_family = "serif") +
  labs(title = "Frequency Distribution", x = "Area Mean", y = "Density") +
  theme(plot.title = element_text(hjust = 0.5, size = 12, face = "bold"), plot.margin = margin(r = 20))
...
```{r qqplot log(y), fig.width=8, out.width="100%", fig.asp=0.5, fig.cap="Normality Assessment on Log(Area)", fig.pos="H"}
...
matrix_df <- df %>% 
  dplyr::select(where(is.numeric), -any_of(c("Diagnosis", "ID", "log_Area_Mean")))

# Partial Correlation Matrix (needs to recapture column/row names)
...
texture1_matrix_pcorr<- pcor_matrix[texture_cols1, texture_cols1]
texture2_matrix_pcorr <- pcor_matrix[texture_cols2, texture_cols2]
...
texture1_matrix_corr<- corr_matrix[texture_cols1, texture_cols1]
texture2_matrix_corr <- corr_matrix[texture_cols2, texture_cols2]
...
```{r matrix-fig-size, fig.width=8, fig.asp=0.5, out.width="100%", fig.cap="Correlation Analysis of Size Metrics", fig.pos="H"}


# Create the Standard Correlation Plot
p1_size <- ggcorrplot(size_matrix_corr, 
                 hc.order = FALSE, type = "lower", lab = TRUE, lab_size = 2.3, title = "Standard Correlation") + shared_theme

# Create the Partial Correlation Plot
p2_size <- ggcorrplot(size_matrix_pcorr, 
                 hc.order = FALSE, type = "lower", lab = TRUE, lab_size = 2.3, title = "Partial Correlation") + shared_theme

# Combine with Patchwork and add a Super-Title for this specific pair
combined_pair_size <- (p1_size + p2_size)

# Print directly to the Plots viewer one by one
combined_pair_size

```

```{r matrix-fig-texture, fig.width=8, fig.asp=0.5, out.width="100%", fig.cap="Correlation Analysis of Texture Metrics", fig.pos="H"}


# Create the Standard Correlation Plot
p1_texture <- ggcorrplot(texture1_matrix_corr, 
                 hc.order = FALSE, type = "lower", lab = TRUE, lab_size = 2.3, title = "Standard Correlation") + shared_theme

# Create the Partial Correlation Plot
p2_texture <- ggcorrplot(texture1_matrix_pcorr, 
                 hc.order = FALSE, type = "lower", lab = TRUE, lab_size = 2.3, title = "Partial Correlation") + shared_theme
